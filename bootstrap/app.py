...
def generate_nginx_config(domain, https_domain=None):
    ...
    # Uses regex location to reliably strip /calendar/ prefix via captured group $1.
    # NOTE: rewrite + variable proxy_pass silently fails to strip the prefix.
    location ~ ^/calendar(/.*) {{
        set $upstream_calendar http://calendar:80;
        proxy_pass $upstream_calendar$1;
    ...
    # === CalDAV/CardDAV direct paths ===
    # Baikal returns hrefs like /dav.php/calendars/... in XML responses.
    # DAVx5/iOS follow these paths AND echo them in request bodies.
    # These MUST route to Baikal directly (no path rewriting) so SabreDAV
    # sees matching URIs in both the request path and request body.
    location /dav.php/ {{
        set $upstream_calendar http://calendar:80;
